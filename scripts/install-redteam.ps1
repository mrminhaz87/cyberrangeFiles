...
# Downloading gives a hastle all the time, so importing pre downloaded iso
# Comment this out if you dare to use the latest version (wink,wink)
Copy-Item "C:\vagrant\resources\windows\atomic_red_team.zip" -Destination $atomicRedTeamRepoPath
...
# Download and unzip a copy of Sliver
Write-Host "$('[{0:HH:mm}]' -f (Get-Date)) Downloading Sliver..."
# GitHub requires TLS 1.2 as of 2/27
[Net.ServicePointManager]::SecurityProtocol = [Net.SecurityProtocolType]::Tls12
$atomicRedTeamDownloadUrl = "https://codeload.github.com/BishopFox/sliver/zip/refs/heads/master"
$atomicRedTeamRepoPath = "C:\Users\vagrant\AppData\Local\Temp\sliver-master.zip"

# Downloading gives a hastle all the time, so importing pre downloaded iso
# Comment this out if you dare to use the latest version (wink,wink)
Copy-Item "C:\vagrant\resources\windows\sliver-master.zip" -Destination $atomicRedTeamRepoPath
Expand-Archive -path "$atomicRedTeamRepoPath" -destinationpath 'c:\Tools\Sliver' -Force
# then continue the rest
...
if (-not (Test-Path $atomicRedTeamRepoPath)) {
  ...
  Write-Host "Sliver was already installed. Moving On."
}
